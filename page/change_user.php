...
if (is_admin() ) {
	if (isset($_GET['Change_User'])) {
		...
		if (isset($_GET['access'])) {
			if ($id = 1) {
				header('Location: ' . $home_url . '/include/admin_panel.php?error="Поменять права доступа админу невозможно"');
				exit();
			}
			$access = $_GET['access'];
		...
		header('Location: ' . $home_url . '/include/admin_panel.php?success="Данные пользователя успешно изменены!"');
	}
}
